...
IMAGE_CACHE_DIR = os.environ.get('IMAGE_CACHE_DIR', 'static/poster_cache')
IMAGE_CACHE_DURATION_DAYS = 30  # Cache 30 hari
IMAGE_CACHE = {}  # In-memory metadata {url: {'path': ..., 'cached_at': ..., 'hits': ...}}

# Create cache directory — skip jika filesystem read-only (e.g. Vercel)
DISK_CACHE_AVAILABLE = False
try:
    os.makedirs(IMAGE_CACHE_DIR, exist_ok=True)
    _test = os.path.join(IMAGE_CACHE_DIR, '.write_test')
    with open(_test, 'w') as f:
        f.write('ok')
    os.remove(_test)
    DISK_CACHE_AVAILABLE = True
except (OSError, PermissionError):
    print("WARNING: Disk image cache tidak tersedia (filesystem read-only). Image akan di-proxy langsung.")
...
def proxy_image():
    ...
    if DISK_CACHE_AVAILABLE and is_image_cached(image_url):
...
def health_check():
    return jsonify({
        'status': 'ok',
        'disk_cache': DISK_CACHE_AVAILABLE,
